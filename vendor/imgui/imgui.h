...
IMGUI_API int GetItemClickedCountWithSingleClickDelay(
    ...
    float delay =
        -1.0f);  // [BETA] building block for disambiguation between
                 // single-click and double-click. Returns 1 on single-click but
                 // delayed by io.MouseSingleClickDelay after mouse release.
...
IMGUI_API bool IsMouseReleasedWithDelay(
    ImGuiMouseButton button,
    float delay =
        -1.f);  // delayed mouse release. Use sparingly. Prefer higher-level
                // helper GetItemClickedCountWithSingleClickDelay(). Generally
                // used with 'delay >= io.MouseDoubleClickTime' + combined with
                // a 'io.MouseClickedLastCount==1' test.
...
enum ImGuiItemFlags_ {
    ...
    ImGuiItemFlags_LiveEditOnInputText =
        1 << 7,  // true     // InputText: apply keyboard edits to backing value
                 // while typing. Otherwise, edits are applied when validating,
                 // tabbing out or losing focus.
    ImGuiItemFlags_LiveEditOnInputScalar =
        1 << 8,  // false    // DragXXX, SliderXXX, InputScalar: apply keyboard
                 // edits to backing value while typing. Otherwise, edits are
...
enum ImGuiInputTextFlags_ {
    ...
    ImGuiInputTextFlags_EnterReturnsTrue =
        1 << 6,  // Return 'true' when Enter is pressed (as opposed to every
                 // time the value was modified). Consider disabling LiveEdit!
                 // or using IsItemDeactivatedAfterEdit() instead!
    ...
    ImGuiInputTextFlags_NoUndoRedo =
        1 << 16,  // Disable undo/redo. Note that input text owns the text data
                  ...
                  // Disable applying output to backing variable while typing.
                  // Same as setting ImGuiItemFlags_LiveEditOnInput to false.
...
enum ImGuiKey : int {
    ...
    ImGuiKey_GamepadFaceLeft,   // X           | Y       | Square   | Toggle
                                // Menu. Hold for Windowing mode
...
enum ImGuiConfigFlags_ {
    ...
    ImGuiConfigFlags_NavEnableKeyboard =
        1 << 0,  // Master keyboard navigation enable flag. Enable full Tabbing
                 ...
                 // regardless of this flag (and may be disabled via other
                 // means, see #4828, #9218).
...
enum ImGuiBackendFlags_ {
    ...
    ImGuiBackendFlags_HasParentViewport =
        1 << 13,  // Backend Platform supports honoring
                  ...
                  // applying the corresponding parent/child relationship at the
                  // Platform level. Child windows always appear in front of
                  // their parent window.
};
...
enum ImGuiColorEditFlags_ {
    ...
    ImGuiColorEditFlags_NoDragDrop =
        1 << 9,  //              // ColorEdit: disable drag and drop
                 //              target/source. ColorButton: disable drag and
                 //              drop source.
...
enum ImGuiTableFlags_ {
    ...
    ImGuiTableFlags_Reorderable =
        1 << 1,  // Enable reordering columns in header row. (Need calling
                 ...
                 // or using ImGuiTableFlags_ContextMenuInBody to access
...
struct ImGuiTableColumnSortSpecs {
    ImGuiID ColumnUserID;  // User data for the column (if specified by a
                           // TableSetupColumn() call in the 'ImGuiID user_data'
                           // field). FIXME: Should be called 'UserData'..
...
struct ImGuiStyle {
    ...
    float SelectableRounding;  // Radius of Selectable rounding. MODIFYING THIS
                               // IS DISCOURAGED. CONTIGUOUS SELECTIONS WILL NOT
                               // LOOK RIGHT. (#7589)
    ...
    float
        SeparatorSize;  // Thickness of border in Separator(). Must be >= 1.0f.
    ...
    bool AntiAliasedFill;  // Enable anti-aliased edges around filled shapes
                           ...
                           // beginning of the frame (copied to ImDrawList).
    float
        CurveTessellationMaxError;  // Maximum error (in pixels) when using
...
struct ImGuiIO {
    ...
    bool
        ConfigViewportsNoDefaultParent;  // = true           // Disable setting
    ...
    bool ConfigInputTextEnterKeepActive;  // = false          // [BETA] Pressing
                                          ...
                                          // Ctrl+Enter or Shift+Enter will
                                          // deactivate normally.
    ImGuiColorEditFlags
        ConfigColorEditFlags;  // = <defaults>     // Current settings for
                               // ColorEdit/ColorPicker widgets. Must have one
                               // bit of ImGuiColorEditFlags_DisplayMask_, one
                               // bit of ImGuiColorEditFlags_DataTypeMask_, one
                               // bit of ImGuiColorEditFlags_PickerMask_, one
                               // bit of ImGuiColorEditFlags_InputMask_.
                               // Defaults to
                               // ImGuiColorEditFlags_DefaultOptions_. May be
                               // further edited by users, unless you also set
                               // ImGuiColorEditFlags_NoOptions.
    ...
    bool ConfigIniSettingsSaveLastUsedDate;  // = true         // Enable
                                             // loading/saving last used day
                                             // (YYYYMMDD) in some .ini struct,
                                             // making things easier to audit
                                             // and allowing custom tools to
                                             // cleanup old data.
    int ConfigIniSettingsAutoDiscardMonths;  // = 0          // [BETA] Set
                                             // number of months after which
                                             // unused .ini entries are
                                             // discarded on load. Require
                                             // platform_io.Platform_SessionDate
                                             // to be set. For systems
                                             // supporting the feature, .ini
                                             // entries without a LastUsed field
                                             // will always be discarded! Please
    ...
    float MouseDoubleClickTime;     // = 0.30f          // Time for consecutive
                                    // clicks to account as a double-click, in
                                    // seconds.
    float MouseDoubleClickMaxDist;  // = 6.0f           // Distance threshold to
                                    // stay in to validate a double-click or
                                    // multiple clicks, in pixels.
    float MouseSingleClickDelay;    // = 0.60f          // Time for a delayed
                                    // click when using
                                    // GetItemClickedCountWithSingleClickDelay()
                                  // or IsMouseReleasedWithDelay(), in seconds.
...
enum ImDrawFlags_ {
    ...
    ImDrawFlags_RoundCornersNone =
        1 << 8,  // Disable rounding even if `float rounding > 0.0f`. This is
...
struct ImDrawList {
    ...
    float _InvFringeScale;   // [internal] 1.0 / _FringeScale // FIXME: Consider
                             // renaming to _PixelDensity.
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    int CmdListsCount;  // Use CmdLists.Size instead. Number of ImDrawList* to
                        // render.
#endif
...
struct ImTextureRect {
    unsigned short x, y;  // Upper-left coordinates of rectangle to update,
                          // within the parent Pixels[] array.
...
struct ImTextureData {
    ...
    void*
        QueueUserData;  // r    -   // Convenience storage for a
                        // staged/multi-threaded rendering texture queue (e.g.
                        // imgui_threaded_rendering.h. See #8597). When != NULL,
                        // core assumes the texture is referenced by the queue.
...
struct ImFontAtlas {
    ...
    IMGUI_API void
    ClearFonts();  // Clear input+output font data/glyphs. New fonts and
                   // textures will be recreated afterwards.
    IMGUI_API void
...
struct ImFontBaked {
    ...
    unsigned int MetricsTotalSurface
        : 26;  // 3  // out // Total surface in pixels to get an idea of the
               // font rasterization/texture cost (not exact, we approximate the
...
struct ImFont {
    ...
    ImU8 Used8kPagesMap[(IM_UNICODE_CODEPOINT_MAX + 1) / 8192 /
                        8];  // 1 bytes if ImWchar=ImWchar16, 17 bytes if
                             ...
                             // of 8K codepoints that has one active glyph. This
...
struct ImGuiViewport {
    ...
    void* PlatformIconData;  // void* to hold custom data structure for the OS /
                             // platform to specify an icon. Currently unused
                             // for exposed to allow experiments.
...
struct ImGuiPlatformIO {
    ...
    int Platform_SessionDate;  // Integer storing YYYYMMDD e.g. 20261231
                               // corresponding to the beginning of application
                               // session.
...
IMGUI_API void SetColorEditOptions(
    ImGuiColorEditFlags
        flags);  // set current options for if you want to select a default
                 // format, picker type, etc. User will be able to change those
